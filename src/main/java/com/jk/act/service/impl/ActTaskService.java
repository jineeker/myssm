...
//		TaskQuery taskQuery = this.taskService.createTaskQuery().taskCandidateOrAssigned(user.getUserAccount().toString());
//		Integer totalSum = taskQuery.list().size();
//		int[] pageParams = page.setTotal(totalSum);
...
//		List<BaseVO> taskList = getBaseVOList(tasks);
		...
		String userId = user.getUserAccount();//ObjectUtils.toString(UserUtils.getUser().getId());
...
//		}
		...
		for (Task task : todoList) {
			...
			BaseVO baseVO = (BaseVO) this.runtimeService.getVariable(processInstance.getId(),"entity");
			if(null==baseVO){
				continue;
			}
			baseVO.setTask(task);
